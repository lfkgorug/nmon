/**
 * lmon.c - Monitor de desempenho baseado em Curses para Unix.
 *
...
/**
 * Use o seguinte Makefile (para Unix em POWER).
...
/**
 * Outros #ifdef's para funcionalidades ou plataformas
 * específicas.
 *
 *     Plataformas:
 *         POWER MAINFRAME X86 ARM. Obrigatório um
 *         deles por vez.
 *
 *     Versões específicas do Unix:
 *         RHEL7 SLES113 SLES12.
 *
 *     Recurso específico:
 *         NVIDIA_GPU.
 *
 *     Soluções alternativas de falhas/ausência
 *     de recursos:
 *         REREAD - para RHEL3.
 *         LSBLK_NO_TYPE - SLES11.3 não tem a opção TYPE
 *         de disco lsblk.
 *
 *     Opções que devem sempre, mas ativadas:
 *         SMALLMEM - remove cubo enorme, não muito limpo,
 *         whritebak, mapeada, slab, pagethreads como não
 *         em kernels mais antigos, um nível de kernel
 *         antes de remover o seguinte para as
...
/**
 * Observação:
 *     O RAW assume que você está usando o índice "i" para
 *     selecionar a CPU.
 */
 ...
 * limpas quando o modo de tela for alterado.
 */
...
/**
 * Para a saída estilo Rain Disk Busy cobrindo
 * centenas de discos em uma tela.
 *
...
/**
 * Relatar dados adicionais de /proc/diskstats
 * para a saída da planilha.
 */
int extended_disk = 0;
...
/**
 * Remover todos os grafemas após o primeiro espaço no
 * retorno de chamada. Remover qualquer '&' logo antes
 * do espaço.
 */
...
/**
 * Remova a saída de falha para este buffer e exiba-a
 * se NMONDEBUG=1.
 */
char errorstr[70];
...
/**
 * lscpu saída de plug para salvar.
 */
    ...
    if (pop != NULL)
    {
        ...
        while (fgets(tmpstr, LSCPU_STRLEN, pop) != NULL)
        {
            ...
            if (strncmp("Architecture:", tmpstr, strlen("Architecture:")) == 0)
            {
                ...
                /**
                 * Comece de grafema após o :<espaço> procurando por
...
/**
 * Número máximo de linhas em páginas /proc.
 * A Intel já tem 26 (portanto aqui 30) por CPU Hypterthread
 * (no máximo 128 * 2 CPUs aqui). O POWER tem apenas 6 a 7
 * linhas, mas recebe 1536 threads SMT em breve.
 * Não é correto no lado seguro abaixo.
 *
...
/**
 * Sinalize se já tivermos o procedimento da função
 * proc_cpu() neste intervalo.
 */
int proc_cpu_done = 0;
...
void proc_read(int num)
{
    ...
    /**
     * Nós relemos P_STAT, agora sinalizamos proc_cpu()
     * que tem que reprocessar esses dados.
     */
    if (num == P_STAT)
    {
        proc_cpu_done = 0;
    ...
    {
        /**
         * Primeira vez, então aloque inicial agora.
         */
    ...
    {
        /**
         * A instalação normal do MAGIC COOKIE WARNING
         * POWER8 pode ter 2655 procedimentos.
         */
    ...
    if (num == P_VERSION)
    {
        ...
        for (i = 0; i < size; i++)
        {
             ...
             * dispositivo.
             *
             * lugar") (" em duas linhas.
             */
            if (found == 0 && proc[num].buf[i] == ')' && proc[num].buf[i + 1] == ' ' && proc[num].buf[i + 2] == '(')
            {
                proc[num].buf[i + 1] = '\n';
                found = 1;
            } else
            {
                 ...
                 * lugar") #" em duas linhas.
                 */
                if (proc[num].buf[i] == ')' && proc[num].buf[i + 1] == ' ' && proc[num].buf[i + 2] == '#')
                {
                    proc[num].buf[i + 1] = '\n';
                }

                /**
                 * Coloque "#1" em duas linhas.
                 */
    ...
    {
        /**
         * Substitua os grafemas de tabulação por espaço.
         */
        if (proc[num].buf[i] == '\t')
        {
            proc[num].buf[i] = ' ';
        ...
        {
            /**
             * Substitua os grafemas de nova linha por null.
             */
            proc[num].lines++;
            proc[num].buf[i] = '\0';
    ...
    /**
     * Defina o sinalizador para que não relemos os dados,
     * mesmo se chamados várias vezes no mesmo intervalo.
     */
...
struct procsinfo
{
    ...
    /**
     * As informações de sinal aqui são obsoletas.
     */
    ...
    /**
     * Páginas não muito limpas.
     */
    unsigned long statm_dt;

    /**
     * Bytes de leitura de armazenamento.
     */
...
/**
 * Usado para calcular a hora/min/segundo.
 */
time_t timer;

/**
 * Contador de recursos.
 * Número de CPUs no sistema (espero que seja mais que zero).
 */
int cpus = 1;
...
/**
 * Dados de arquivo de partições forma abreviada (ou seja,
 * dados ausentes).
 */
...
/**
 * Parar após este número de atualizações.
 */
int maxloops = -1;
 ...
 * que não ultrapasse a linha de entrada da planilha
 * no máximo.
 */
#define DPL 150
...
/**
 * Estatísticas de disco mostra discos somente se a
 * pessoa usou -g auto.
 */
...
/**
 * Suporte a Cursores.
 * Use isso apenas para chamadas de curses de linha única.
 */
#define CURSE if(cursed)

/**
 * Use isso apenas para chamadas de Cursor de cor
 * de linha única.
 */
...
/**
 * 1 = Usando cursores e,
 * 0 = Saída de log para uma planilha.
 */
int cursed = 1;

/**
 * 1 = Usando cursores de cores e.
 * 0 = Usando cursores em preto e branco (Consulte -b flag).
 */
int colour = 1;
...
/**
 * Filepointer para saída de planilha.
 */
FILE *fp;
...
/**
 * Coisas completas do Args Mode aqui.
 */
...
void args_output(int pid, int loop, char *progname)
{
    ...
    } else
    {
        ...
        {
            /**
             * Jogar fora a página de cabeçalho.
             */
            pclose(pop);
        ...
        {
            /**
             * Remover coisas que não era correto no Excel, ou seja,
             * começando com o símbolo de matemática.
             */
            ...
            if (tmpstr[i] == ' ' && tmpstr[i + 1] == ' ')
            {
                ...
                /**
                 * Digitalizar novamente para remover o espaço triplo, etc.
                 */
                i--;
            }
        }
...
/**
 * Finalize as coisas do modo args aqui.
 */
void linux_bbbp(char *name, char *cmd, char *err)
{
    ...
    } else
    {
        ...
        /**
         * 2048 = verificação de sanidade apenas.
         */
        for (i = 0; i < 2048 && (fgets(str, STRLEN, pop) != NULL); i++)
        {
            ...
            /**
             * Corrigir grafemas iniciais de texto que confundem
             * planilhas.
             */
            if (str[0] == '+')
            {
            ...
            /**
             * Remova as aspas duplas, pois não são muito corretas em
             * planilhas.
             */
            for (j = 0; str[j] != 0; j++)
            {
...
/**
 * Nome global do programa para imprimi-lo.
 */
char *progname;
...
/**
 * Estrutura de dados principal para estatísticas
 * coletadas. Duas versões são dados anteriores e
 * atuais. Muitas vezes é a diferença que é impressa.
 * Os ponteiros são trocados, ou seja, atual se torna
 * o anterior e o anterior sobrescrito em vez de
 * mover os dados.
 */
struct cpu_stat
{
    /**
     * Mudou a ordem aqui para corresponder ao kernel deste
     * ano (man 5 /proc/stat).
     */
    ...
    /**
     * Abaixo estão os números não baseados em CPU
     * no mesmo arquivo.
     */
    long long intr;
...
struct nfs_stat
{
    ...
    /**
     * Versão 2 do FORNECEDOR.
     */
    long v2s[NFS_V2_NAMES_COUNT];

    /**
     * Versão 3 do FORNECEDOR.
     */
    long v3s[NFS_V3_NAMES_COUNT];

    /**
     * Versão 4 do FORNECEDOR.
     */
 ...
 */
    ...
    /**
     * Preciso testar se rewind() trabalhou ou não para lparcfg.
     */
    int lparcfg_reread = 1;

    /**
     * Redefina no final de cada intervalo para que LPAR cfg
     * seja lido apenas uma vez em cada intervalo, mesmo se
     * proc_lparcfg() for chamado várias vezes.
     *
     * Observação: lparcfg não é lido através de proc_read().
     */
    ...
    struct {
        ...
        /**
         * O novo registro é "IBM pSeries (emulado por qemu)"
         * em vez de "IBM 9119-MME".
         *
         * 11.
         */
        int partition_id;
        ...
        /**
         * Falhas de página do hipervisor = grande número.
         */
        long cmo_faults;
        ...
        /**
         * Tempo do hipervisor em micro segundos = grande.
         */
        ...
        /**
         * Tamanho total do pool AMS em bytes.
         */
        long entitled_memory_pool_size;

        /**
         * AMS solicitando mais busca de cubos.
         */
        ...
        #ifdef EXPERIMENTAL
            /**
             * Novos dados no SLES11 para POWER 2.6.27 (pode
             * ser um pouco anterior também).
             */
    ...
    /**
     * O retorno de 0 significa que os dados não estão
     * disponíveis.
     */
    int proc_lparcfg()
    {
        ...
        /**
         * Apenas tente ler /proc/ppc64/lparcfg uma vez - lembre-se
         * se é legível.
         */
        ...
        if (fp == (FILE *) - 1)
        {
            if ((fp = fopen("/proc/ppc64/lparcfg", "r")) == NULL)
            {
                ...
                {
                    /**
                     * Heurística para detectar Host PowerKVM.
                     *     a) Dentro do ifdef POWER, então não pode ser x86.
                     *     b) /proc/ppc64/lparcfg está faltando.
                     *     c) /etc/ *ease arquivos têm dicas.
                     *
                     * Verdadeiro confirmado para IBM_POWERKVM 2.1.
                     */
        ...
        {
            /**
             * Não está claro se close+open é necessário.
             * Infelizmente, isso requer a instalação de muitas versões do
             * Unix em POWER para testar os primeiros lançamentos.
             */
            fclose(fp);
            fp = (FILE *) - 1;
        } else
        ...
        {
            /**
             * Remova novos espaços no significado de string PowerKVM.
             */
            if (str[i] == ' ')
            {
                str[i] = '-';
        ...
        {
            /**
             * Heurística para localizar convidados PowerKVM.
             *     a) Dentro do ifdef POWER, então não pode ser x86.
             *     b) Temos um /proc/ppc64/lparcfg - provavelmente ausente (1.9).
             *     c) string de tipo de sistema inclui "qemu".
             *
             * Verdadeiro confirmado para SLES, RHEL e Ubuntu.
             */
         ...
         *
         */
        #define GETDATA(variable) \
            lparcfg.variable = read_longlong(__STRING(variable));
        ...
        if (lparcfg.cmo_enabled)
        {
            ...
            /**
             * Tempo do hipervisor em microssegundos = grande número.
             */
            ...
            /**
             * AMS solicitando mais busca do cubo.
             */
...
/**
 * Suporta até 780, mas não POWER6 595 acompanhamento
 * com POWER7. Precisa ser re-observado para lidar
 * com números mais altos, raros, mas interessantes,
 * de máquinas de CPU.
 *
 * AVISO DE COOKIE MÁGICO.
 */
 ...
 *
 */
#define GETVM(variable) \
    p->vm.variable = get_vm_value(__STRING(variable));
...
int read_vmstat()
{
     ...
     * em /proc/vmstat, ela será definida como -1.
     */
...
/**
 * Estas macros simplificam o acesso à estrutura
 * de dados Principal.
 */
...
/**
 * Verificação de plug em uma lista.
 */
#define CMDMAX 64
...
/**
 * Converter segundos + micro segundos para um duplo.
 */
...
void get_cpu_cnt()
{
    ...
    /**
     * Obter informações da CPU de /proc/stat e aumentar
     * proc[P_STAT].
     */
    proc_read(P_STAT);

    /**
     * Comece com o índice [1], pois [0] contém as estatísticas
     * gerais da CPU.
     */
 ...
 */
    ...
    void get_intel_spec()
    {
        ...
        /**
         * Obter informações da CPU de /proc/stat e aumentar proc[P_STAT].
         */
        proc_read(P_CPUINFO);
        ...
        /**
         * Comece com o índice [1], pois [0] contém as
...
void proc_cpu()
{
    ...
    } else
    {
        ...
        {
            /**
             * stat 4 variáveis aqui como proc Unix mais antigo.
             */
    ...
    /**
     * Se tivéssemos uma mudança no número de CPUs, copiamos
     * os dados do intervalo atual para o anterior, assim
     * obtemos um intervalo de utilização "0", mas melhor
     * que negativo ou 100%.
     *
     * Alerta - Isso afeta as mudanças do POWER SMT também.
     */
    if (old_cpus != cpus)
    {
    ...
    /**
     * Sinalizar que rotinas dos dados de /proc/stat; redefinido
...
void proc_nfs()
{
    ...
    if (proc[P_NFS].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFS].lines; lineno++)
        {
            ...
            {
                /**
                 * Leitores de linha do cliente versão 2 "proc2 18 num num etc".
                 */
            ...
            {
                /**
                 * Versão de cliente 3 leitores de linha "proc3 22 num num etc".
                 */
                len = strlen(proc[P_NFS].line[lineno]);
            ...
            {
                /**
                 * Versão do cliente 4 leitores de linha "proc4 35 num num etc".
                 */
    ...
    if (proc[P_NFSD].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFSD].lines; lineno++)
        {
            ...
            {
                /**
                 * Versão do fornecedor de dois leitores de linha "proc2 18
                 * num num etc".
                 */
            ...
            {
                /**
                 * Versão do fornecedor de três leitores de linha
                 * "proc3 22 num num etc".
                 */
                len = strlen(proc[P_NFSD].line[lineno]);
            ...
            {
                /**
                 * Versão do fornecedor de quatro leitores de linha
                 * "proc4ops 40 num num etc". Observação: O "ops",
                 * portanto, começando na pilastra 9.
                 */
...
void proc_disk_io(double elapsed)
{
    ...
    for (i = 0; i < disks; i++)
    {
        ...
        /**
         * Suponha que um disco faz 200 procedimentos por segundo.
         */
...
void proc_diskstats(double elapsed)
{
    ...
    for (i = 0; i < DISKMAX;)
    {
        ...
        /**
         * Embaralhe os dados devido à falta de colunas para
         * partições.
         */
        if (ret == 7)
        {
        ...
        /**
         * Em milissegundos para chegar até 100%, 1000/100 = 10.
         */
        p->dk[i].dk_time /= 10.0;
...
void proc_partitions(double elapsed)
{
    ...
    {
        /**
         * Jogue fora as linhas de cabeçalho.
         */
        goto end;
    }
    ...
    /**
     * Maior Menor: #blocks name rio rmerge rsect ruse wio
     * wmerge wsect wuse com procedimentos use aveq.
     *
     *
    ...
    for (i = 0; i < DISKMAX; i++)
    {
        ...
        /**
         * Em milissegundos para chegar a 100%, 1000/100 = 10.
         */
        p->dk[i].dk_time /= 10.0;
...
void proc_mem()
{
     ...
     * discos CUBO em kernels posteriores.
     */
    ...
    if (cursed)
    {
        ...
        for (j = 0; j < MAX_SNAPS; j++)
        {
            for (i = 0; i < MAX_SNAP_ROWS; i++)
            {
                ...
                {
                    if ((cpu_snap[j].user + cpu_snap[j].kernel + cpu_snap[j].iowait) / 100 * MAX_SNAP_ROWS > i + 0.5)
                    {
...
void plot_smp(WINDOW * pad, int cpu_no, int row, double user, double kernel, double iowait, double idle, double steal)
{
    ...
    if (cursed)
    {
        ...
        {
            /**
             * Adicionar "=" para tentar compensar as metades
             * que faltam.
             */
            #ifdef POWER
...
/**
 * Variável adicionada para lembrar as camadas mais baixas
 * já iniciadas.
 *     0 - start
 *     1 - snap
 *     2 - end
...
void init_pairs()
{
    ...
    /**
     * Plano de Fundo Ciano, Texto Ciano.
     */
...
/**
 * Editor de Sinal.
 *     SIGUSR1 ou 2 é usado para parar o nmon de forma limpa.
...
void interrupt(int signum)
{
    ...
    {
        /**
         * Cursor (stop + start) para que funcione o número de
         * linhas e pilastras.
         */
        CURSE endwin();
...
/**
 * Coloque apenas os ponteiros q=anterior e p=atual
 * são modificados.
 */
void switcher(void)
{
    ...
    /**
     * Redefinir sinalizadores para que /proc/... seja
...
/**
 * Pesquise a string de estado do procedimento correto.
 */
...
/**
 * Grupos de discos definidos pela pessoa.
 */
...
void load_dgroup(struct dsk_stat *dk)
{
    ...
    {
        /**
         * Ignorar as linhas que começam com #.
         */
        if (line[0] == '#')
        {
            /**
             * Foi uma linha de comentário.
             * Diminuir dgroup_total_groups em 1 para reparar o
             * índice para o próximo loop.
             *
             */
            --dgroup_total_groups;
            continue;
        ...
        for (i = 0; i < DGROUPITEMS && *nextp != 0; i++)
        {
            ...
            for (j = 0; j < disks; j++)
            {
                if (strcmp(dk[j].dk_name, name) == 0)
                {
                    /**
                     * DEPURAÇÃO:
                     *     printf("DGadd group=%s,name=%s,disk=%s,dgroup_total_groups=%d,dgroup_total_disks=%d,j=%d,i=%d,index=%d.\n",
...
void list_dgroup(struct dsk_stat *dk)
{
    ...
    for (n = 0, i = 0; i < dgroup_total_groups; i++)
    {
        ...
        for (k = 0, j = 0; j < dgroup_disks[i]; j++)
        {
            ...
            /**
             * Adicionar linha extra se tivermos muito para parar as
             * falhas de largura de linha da planilha.
             */
            if (k == 128)
            {
...
void jfs_load(int load)
{
    ...
    /**
     * FILE ponteiro para arquivo mtab.
     */
    FILE *mfp;

    /**
     * Estatísticas de ponto mnt.
     */
    struct mntent *mp;
    ...
    if (load == LOAD)
    {
        ...
        /**
         * Primeiro, ou mais tarde - basta reabrir os
...
/**
 * Nós pedimos esse vetor em vez das tabelas de procedimentos comum.
 *     - O índice é a posição na tabela de procedimentos e o
 *       tamanho é o cubo usado em bytes.
 *     - O io é o storge I/O realizado no último período em bytes.
...
/**
 * Rotina usada pelo qsort para ordenar os procedimentos
 * pelo uso da CPU.
 */
...
int checkinput(void)
{
    ...
    {
        /**
         * Sem entrada da pessoa, então não deixe de usar
         * um control-C.
         */
        return 0;
    }
        ...
        if (chars > 0)
        {
            ...
            for (i = 0; i < chars; i++)
            {
                switch (buf[i])
                {
                    ...
                    /**
                     * Limite a saída para procedimentos e discos realmente
                     * trabalhando.
                     */
                    case '.':
                        if (show_all)
                        ...
                        } else
                        {
                            ...
                            /**
                             * Mudar para a visualização favorita de Bom Exemplo
                             * é não muito discreto para os outros, então
                             * desative esse recurso.
...
void proc_net()
{
    ...
    {
        /**
         * Remover linhas do cabeçalho de páginas.
         */
        goto end;
    }
    ...
    /**
     * +------------+---------------------------------------------------------------- ------------+-----------------------------------------------------------------------------------+
     * | Inter-     | Obter                                                                       | Transmitir.                                                                       |
     * +------------+-------+---------+--------+---------+------+--------+----------- +-----------+---------+---------+--------+---------+------+-----------+------------+------------+
     * | superfície | bytes | plugs   | falhas | remover | fifo | quadro | compactado | multicast | bytes   | plugs   | falhas | remover | fifo | pilastras | fornecedor | compactado |
     * +------------+-------+---------+--------+---------+------+--------+------------+-----------+---------+---------+--------+---------+------+-----------+------------+------------|
    ...
    for (i = 0; i < NETMAX; i++)
    {
         ...
         */
        ret = sscanf(&buf[0], "%s %llu %llu %lu %lu %lu %lu %lu %lu %llu %llu %lu %lu %lu %lu %lu", (char *) &p->ifnets[i].if_name,
...
int proc_procsinfo(int pid, int index)
{
    ...
    /**
     * Agora procure por ") " como driver Infiniban "dumb" que inclui "()".
     */
    ...
    /**
     * Feche-o mesmo se a leitura falhar, a página pode ter sido
     * removida entre abrir e ler, ou seja, o driver do
...
int getprocs(int records)
{
    ...
    while ((char *) (dent = readdir(procdir)) != NULL)
    {
        ...
        {
            /**
             * Isso é um directório.
             * Descrição de mainframes 0 = sempre desconhecido.
             */
            if (isnumbers(dent->d_name))
            {
                ...
                {
                    /**
                     * Apenas contadores do modo de procedimentos.
                     */
                    count++;
                }
            }
...
/**
 * Inicie o procedimento conforme especificado no cmd em
 * um procedimento inicial sem aguardar a conclusão, não
 * tenho certeza se deseja impedir essa funcionalidade
 * para a uma pessoa com níveis de permissão do sistema
 * mais avançado.
 *
 * Quando: CHLD_START, CHLD_SNAP ou CHLD_END.
 *     CMD: Ponteiro para string de shell - considerado limpo.
 *     timestamp_type: 0 - T%04d, 1 - carimbo de tempo detalhado.
 *     ciclo: id do ciclo (0 para CHLD_START).
...
void child_start(int when, char *cmd, int timestamp_type, int loop, time_t the_time)
{
    ...
    /**
     * Validação do parâmetro e inicialização do texto
     * da falha.
     */
    switch (when)
    {
        ...
        case CHLD_SNAP:
            /**
             * Verifique se a rotina de baixo nível, com maior
             * tempo de uso já terminou - caso contrário, não
             * fazemos nada.
             */
    ...
    /**
     * Agora vamos obter uma cópia de uma rotina de baixo
     * nível.
     */
    switch (child_pid = fork())
    {
        /**
         * Rotina de cópia falhou.
         */
        case -1:
            perror(when_info);
            return;
        ...
        /**
         * Na rotina de baixo nível.
         */
        case 0:
            /**
             * Criar carimbo de data/hora solicitado.
             */
            if (timestamp_type == 1)
            {
            ...
            /**
             * Terminar todas as rotinas de cubo de páginas em aberto,
             * exceto as especificações.
             */
            for (i = 3; i < 5; ++i)
            {
                close(i);
            ...
            /**
             * Agora mude para o plug definido.
             */
            execlp(cmd, cmd, time_stamp_str, (void *) 0);

            /**
             * Se chegarmos aqui, o plug especificado não pôde
             * ser iniciado.
             */
            perror(when_info);
        ...
        default:
            /**
             * Dentro da rotina da camada mais alta.
             * Na camada mais alta - lembre-se da camada mais baixa
             * pid para o futuro.
             */
...
int main(int argc, char **argv)
{
    ...
    /**
     * Observe que tipo de carimbo de data/hora damos as
     * camadas inferior já iniciadas.
     *     0: "T%04d".
    ...
    /**
     * Pontos do relógio por segundo usados em /proc/stat linhas de cpu.
     */
    long ticks = 100;

    /**
     * O tamanho de página normal é de 4KB, mas os fornecedores
    ...
    /**
     * Abaixo é usado para treinar a rotina nmon, obter-la e
     * permitir o segundo de espera para reduzir o desvio de
     * segundo.
     */
    ...
    /**
     * Alterado para permitir larguras máximas de pilastras.
     */
    #define MAXCOLS 150
    ...
    #define DISPLAY(pad, rows) \
        ...
        /**
         * Verifique as opções fornecidas pela pessoa.
         */
        progname = argv[0];
        ...
        /**
         * Controles do Coletor de Sequências de Cubos Externos.
         */
        ...
        /**
         * Configurar nome de host com mais e menos bytes.
         */
        ...
        /**
         * Obtenha os pontos do relógio por segundo para
         * contadores de CPU em /proc/stat cpu stats.
         */
        ...
        /**
         * Verifique se temos os grandes tamanhos de página de
         * cubos de 64KB compilados no kernel.
         */
        ...
        while (-1 != (i = getopt(argc, argv, "?abc:C:Dd:EfF:g:hI:Jl:m:MNpr:Rs:tTUVxXz")))
        {
            switch (i)
            {
                ...
                /**
                 * Modo de segundo plano com nome de arquivo fornecido
                 * pela pessoa.
                 */
                case 'F':
                ...
                /**
                 * Modo de segundo plano, ou seja, para saída de planilha.
                 */
                case 'f':
                    go_background(288, 300);
                    break;
                ...
                case 'g':
                    ...
                    if (strncmp("auto", dgroup_filename, 5) == 0)
                    {
                        ...
                        #ifdef SLES113
                            /**
                             * Defina isso para contornar a falta do recurso
                             * --output TYPE.
                             */
            ...
            case 't':
                /**
                 * Colocar a saída do procedimento principal no modo
                 * de planilha.
                 */
                show_top = 1;
            ...
            /**
             * Modo de segundo plano para planejamento da capacidade
             * de 1 dia.
             */
            case 'x':
            ...
            case 'X':
                ...
                /**
                 * Modo de segundo plano para saída de 1 dia para
                 * /var/perf/tmp
                 */
                varperftmp++;
                go_background(4 * 24, 15 * 60);
    ...
    /**
     * Para obter a configuração dos ponteiros.
     */
    switcher();

    /**
     * Inicialize os timestamps para o primeiro ciclo.
     */
    p->time = doubletime();
    q->time = doubletime();
    ...
    /**
     * Determine o número de CPUs LÓGICAS ativas -
     * depende do modo SMT.
     */
    get_cpu_cnt();
    ...
    /**
     * Inicialize a tabela de procedimentos principais.
     */
    topper_size = n;

    /**
     * Arredondamento para o topo.
     */
    ...
    /**
     * Se estivermos fazendo um procedimento no modo de
     * planilha, inicialize todos os outros conjuntos de
     * dados também para não obtermos dados não muito
     * corretos para o primeiro intervalo relatado.
     */
    if (!cursed)
    {
        /**
         * Obter estatísticas de VM.
         */
        read_vmstat();

        /**
         * Obter informações do cubo.
         */
        proc_mem();

        #ifdef POWER
            /**
             * Obtenha estatísticas de LPAR.
             */
            proc_lparcfg();
        #endif
    }
    ...
    /**
     * Defina o ponteiro pronto para a próxima rodada.
     */
    switcher();

    /**
     * Inicialize os alternadores de sinal para que
     * possamos arrumar cursor na saída.
     */
    ...
    {
        /**
         * Gere as linhas de cabeçalho da planilha.
         */
        timer = time(0);
        tim = localtime(&timer);

        /**
         * Ler a página de manual localtime().
         */
        ...
        if (varperftmp)
        {
            ...
            /**
             * Nome do host mais tamanho do diretório mais o número.
             */
        ...
        if (!debug)
        {
            ...
            /**
             * Tornar um ponto do grupo de procedimentos.
             */
            setpgrp();
        ...
        /**
         * Faça a atividade nmon_start desde o início.
         */
        if (nmon_start)
        {
            timer = time(0);
        ...
        if (show_aaa)
        {
            ...
            fprintf(fp, "AAA,snapshots,%d\n", maxloops);
                ...
                /**
                 * Dê ao analisador a chance de encontrar facilmente
                 * o comprimento do número da CPU - 3 dígitos aqui.
                 */
        ...
        /**
         * iremoved porque não está abaixo na linha BUSY fprintf(fp,"\n");
         */
        #ifdef DEBUG
            if (debug)
            {
        ...
        if (show_jfs)
        {
            ...
            {
                /**
                 * /proc dá valores não muito corretos.
                 */
                if (jfs[k].mounted
        ...
        /**
         * Para obter as primeiras estatísticas para cobrir este
         * segundo e evitar falhas de divisão por zero.
         */
        sleep(1);
    }
    ...
    /**
     * Para obter a configuração dos ponteiros.
     * Já foi feito anteriormente, NÃO volte aqui para o ponteiro
     * antigo. - switcher();
     * checkinput();
     */
    clear();
    ...
    /**
     * Ciclo principal do código.
     */
    for (loop = 1;; loop++)
    {
        /**
         * Economize tempo e calcule quanto tempo realmente pausamos.
         * Faça isso o mais cedo possível e perto de ler as estatísticas
         * da CPU em /proc/stat.
         */
        p->time = doubletime();
        ...
        #ifdef POWER
            /**
             * Sempre obtenha informações de lpar também para que
             * possamos relatar o uso físico da CPU para tornar
             * os dados mais significativos. O valor de retorno
             * é ignorado aqui, mas lembrado em proc_lparcfg().
             */
            proc_lparcfg();
        #endif

        /**
         * Isso interrompe o nmon causando o escalonamento da
         * CPU na inicialização.
         */
        if (loop <= 3)
        {
            for (i = 0; i < max_cpus + 1; i++)
        ...
        /**
         * Redefina a posição do cursor para o canto mais alto
         * esquerdo.
         */
        y = x
          = 0;
                ...
                COLOUR wattrset(padwelcome, COLOR_PAIR(1));
                    ...
                    switch (power_vm_type)
                    {
                        ...
                        case VM_POWERVM:
                            ...
                            mvwprintw(padwelcome, x + 10, 3, "PowerVM Entitlement=%-6.2f VirtualCPUs=%d LogicalCPUs=%d", (double) lparcfg. partition_entitled_capacity / 100.0, (int) lparcfg.partition_active_processors, cpus);
        ...
        if (show_res && cursed)
        {
            ...
            COLOUR wattrset(padres, COLOR_PAIR(3));
                ...
                /**
                 * Precisa que o lparcfg já tenha um procedimento.
                 */
                proc_lparcfg();
                ...
                switch (power_vm_type)
                {
                    ...
                    case VM_POWERKVM_HOST:
                        mvwprintw(padres, 9, 20, "PowerKVM Host Physical CPU:%d  %s",
                            cpus,
                            lscpu.byte_order);
                        break;
        ...
        if (show_smp || show_verbose || show_wide)
        {
            ...
            if (old_cpus != cpus)
            {
                ...
                } else
                {
                     ...
                     * wmove(padsmp,0,0);
                     *
                     * Sem cursor - wclrtobot(padsmp);
                     * Tentar sobrescrever os dados anteriores.
                     */
                    if (cpus < old_cpus)
                    {
            ...
            if (show_smp)
            {
                ...
                if (cursed)
                {
                     ...
                     *
                     * Isso pressupõe que as informações de LPAR
                     * estejam disponíveis em q e p.
                     */
                    if (proc_lparcfg() > 0)
                    {
                        if (lparcfg.shared_processor_mode == 1)
                        {
                            ...
                            {
                                /**
                                 * Host ou Convidado PowerKVM ou Nativo não têm
                                ...
                                /**
                                 * Só se o cálculo estiver funcionando.
                                 */
                ...
                for (i = 0; i < cpus; i++)
                {
                    ...
                    /**
                     * Verifique se tivemos uma mudança de CPU e temos
                     * que definir ocioso para 100.
                     */
                    if (cpu_sum == 0)
                    {
                ...
                CURSE mvwprintw(padsmp, i + 3, 0, cpu_line);
                    ...
                    if (lparcfg.shared_processor_mode == 1)
                    {
                        ...
                        } else
                        {
                            ...
                            /**
                             * PowerKVM não tem conceito limitado.
                             */
                            if (power_vm_type == VM_POWERVM)
                            {
        ...
        if (show_util)
        {
            ...
            } else
            {
                ...
                for (i = 0; i < cpus; i++)
                {
                    fprintf(fp, "CPUUTIL%03d,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n", i, LOOP,
                        RAW(user) / elapsed,
                        RAW(nice) / elapsed,
                        RAW(sys) / elapsed,
                        RAW(idle) / elapsed,
            ...
            if (show_lpar)
            {
                ...
                {
                    /**
                     * Imprima apenas as informações da LPAR na planilha se
                     * estiver no modo de procedimento compartilhado.
                     */
                    if (ret != 0 && (lparcfg.shared_processor_mode > 0 || lparcfg.DedDonMode > 0) && power_vm_type == VM_POWERVM)
                    {
                        /**
                         * cpus, ~> relatar a CPU lógica aqui para que o gráfico do
                         * analisador % de CPU vs VPs faça o retorno corretamente.
                         */
                ...
                } else
                {
                    ...
                    } else
                    {
                        ...
                        if (VMCOUNT(nr_slab) != -1 )
                        {
                             ...
                             */
                            mvwprintw(padpage, 6, 0, "nr_slab     =%9lld pgactivate  =%8lld", VMCOUNT(nr_slab), VMDELTA(pgactivate));
                            mvwprintw(padpage, 7, 0, "                       pgdeactivate=%8lld", VMDELTA(pgdeactivate));
                        } else
                        {
                            /**
                             * Novo em: nr_slab_reclaimable/nr_slab_unreclaimable Kernel.
                             */
                        ...
                        mvwprintw(padpage, 8, 0, "allocstall  =%9lld pgfault     =%8lld  kswapd_steal     =%7lld", VMDELTA(allocstall), VMDELTA(pgfault), VMDELTA(kswapd_steal));
                        mvwprintw(padpage, 9, 0, "pageoutrun  =%9lld pgmajfault  =%8lld  kswapd_inodesteal=%7lld", VMDELTA(pageoutrun), VMDELTA(pgmajfault), VMDELTA(kswapd_inodesteal));
                        mvwprintw(padpage, 10, 0, "slabs_scanned=%8lld pgrotated   =%8lld  pginodesteal     =%7lld", VMDELTA(slabs_scanned), VMDELTA(pgrotated), VMDELTA(pginodesteal));
                        mvwprintw(padpage, 1, 46, "              High Normal    DMA");
                        mvwprintw(padpage, 2, 46, "alloc      %7lld%7lld%7lld", VMDELTA(pgalloc_high), VMDELTA(pgalloc_normal), VMDELTA(pgalloc_dma));
                        mvwprintw(padpage, 3, 46, "refill     %7lld%7lld%7lld", VMDELTA(pgrefill_high), VMDELTA(pgrefill_normal), VMDELTA(pgrefill_dma));
                        mvwprintw(padpage, 4, 46, "steal      %7lld%7lld%7lld", VMDELTA(pgsteal_high), VMDELTA(pgsteal_normal), VMDELTA(pgsteal_dma));
                        mvwprintw(padpage, 5, 46, "scan_kswapd%7lld%7lld%7lld", VMDELTA(pgscan_kswapd_high), VMDELTA(pgscan_kswapd_normal), VMDELTA(pgscan_kswapd_dma));
                        mvwprintw(padpage, 6, 46, "scan_direct%7lld%7lld%7lld", VMDELTA(pgscan_direct_high), VMDELTA(pgscan_direct_normal), VMDELTA(pgscan_direct_dma));
                    }
                }
        ...
        if (show_kernel)
        {
            ...
            } else
            {
                ...
                /**
                 * Esses "-1" não parecem muito bons, mas mantêm o formato
                 * nmon para AIX. As estatísticas não estão disponíveis no
                 * UNIX. . . a menos que o famoso saiba melhor. Más sem
                 * nenhuma permissão aqui.
                 *
                 * Parâmetros:
                 *     ~> fp.
                 *     ~> str_p.
                 *     ~> LOOP.
                 *     ~> (float) p->cpu_total.running. // (- 1) fila de procedimentos.
                 *     ~> (float) p->cpu_total.blocked. // 2 swapin (número de procedimentos
                 *                                      // aguardando a conclusão do IO.
                 *     ~> (float) (p->cpu_total.ctxt - q->cpu_total.ctxt) / elapsed // 3 plugs.
                 *     ~> -1.0. // 4 chamadas de sistema.
                 *     ~> -1.0. // 4 leitura.
                 *     ~> -1.0. // 5 grafia.
                 *     ~> (float) (p->cpu_total.procs - q->cpu_total.procs) / elapsed // 6 copiá.
                 *     ~> -1.0 // 7 procedimento.
                 *     ~> -1.0 // 8 sem.
                 *     ~> -1.0 // 9 msg.
                 */
            ...
            if (cursed)
            {
                ...
                if (nfs_v4c_found)
                {
                    for (i = 0; i < 18; i++)
                    {
                        ...
                        if (show_nfs == 2)
                        {
                            mvwprintw(padnfs, 2 + i, 0, "%12s%7.1f", nfs_v4c_names[i], NFS_DELTA(nfs.v4c[i]));
                        }
                    ...
                    for (i = 18; i < 35; i++)
                    {
                        ...
                        if (show_nfs == 2)
                        {
                            mvwprintw(padnfs, 2 + i - 18, 20, "%12s%7.1f", nfs_v4c_names[i], NFS_DELTA(nfs.v4c[i]));
                        }
                ...
                if (nfs_v4s_found)
                {
                    for (i = 0; i < 18; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i, 0, "%12s%7.1f", nfs_v4s_names[i], NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 18; i < 36; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 18, 19, "%12s%7.1f", nfs_v4s_names[i], NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 36; i < 54 && i < nfs_v4s_names_count; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 36, 39, "%12s%7.1f", nfs_v4s_names[i], NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 54; i <= 70 && i < nfs_v4s_names_count; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 54, 59, "%12s%7.1f", nfs_v4s_names[i], NFS_DELTA(nfs.v4s[i]));
                        }
        ...
        if (show_jfs)
        {
            if (cursed)
            {
                ...
                for (k =0, j = 0; k < JFSMAX && j < jfses; k++)
                {
                    ...
                    /**
                     * !strncmp(jfs[k].name, "/proc/", 6) ~> subdiretórios também devem ser não.
                    ...
                    {
                        /**
                         * /proc dá valores invalid/insane.
                         */
                        if(show_jfs_minimum)
                        {
                            /**
                    ...
                    } else
                    {
                        ...
                        if ((ret = fstatfs(jfs[k].fd, &statfs_buffer)) != -1)
                        {
                            ...
                            {
                                /**
                                 * O Unix não muito recente parecia sempre relatar
                                 * em blocos de 4KB, mas a versão mais recente do
                                 * Unix usa os tamanhos de bloco f_bsize, mas o
                                 * man statfs tem o manual do campo como o tamanho
                                 * natural de E/S, portanto, os blocos relatados
                                 * aqui são ambíguos em tamanho.
                                 */
                                ...
                                /**
                                 * Converter blocos para MB.
                                 */
                                fs_size = (float) statfs_buffer.f_blocks * fs_bsize / 1024.0 / 1024.0;

                                /**
                                 * Encontre as melhores informações de tamanho disponíveis
                                 * f_bavail é como relatórios df caso contrário, use
                                 * f_bsize (isso inclui blocos inode).
                                 */
            ...
            } else
            {
                ...
                for (k = 0; k < jfses; k++)
                {
                    ...
                    {
                        /**
                         * Valores /proc gives invalid/insane.
                         */
                        if (fstatfs(jfs[k].fd, &statfs_buffer) != -1)
                        {
                            ...
                            {
                                /**
                                 * Alguns pseudo sistemas de páginas têm tamanho zero,
                                 * mas obtemos um resultado -nan.
                                 */
                                fs_size_used = 0.0;
                            } else
                            {
                ...
                if (cursed)
                {
                    ...
                    } else
                    {
                        ...
                        for (i = 0, k = 0; i < disks; i++)
                        {
                            ...
                            {
                                /**
                                 * Estatísticas de discos de saída.
                                 */
                            ...
                            {
                                /**
                                 * Gráficos de barra de disco de saída.
                                 */
                                if (disk_mb)
                                {
                                ...
                                {
                                    /**
                                     * 50 pilastra na área do gráfico do disco, então
                                     * faça uma divisão de % por dois.
                                     */
                                ...
                                /**
                                 * Verifique se o arredondamento não tem o pico ">"
                                 * acima de 100%.
                                 */
                ...
                if (cursed)
                {
                    ...
                    } else
                    {
                        ...
                        for (k = n = 0; k < dgroup_total_groups; k++)
                        {
                             ...
                             * Em MB.
                             */
                            mvwprintw(paddg, n + 2, 1, "%-14s   %3d %5.1f%%          |          %6.1f %9.1f %6.1f ", dgroup_name[k], dgroup_disks[k], disk_busy, (disk_read + disk_write) / 1024, disk_xfers, disk_size);
                            COLOUR wattrset(paddg, COLOR_PAIR(6));
                         ...
                         * Em MB.
                         */
                        mvwprintw(paddg, n + 2, 1, "Groups=%2d TOTALS %3d %5.1f%% %9.1f|%-9.1f %6.1f %9.1f", n, dgroup_total_disks, total_busy / dgroup_total_disks, total_rbytes, total_wbytes, (((double) total_rbytes + (double) total_wbytes)) / 1024, total_xfers);
                    }
            ...
            if (show_top)
            {
                ...
                /**
                 * Obtenha os detalhes dos procedimentos em serviço.
                 */
                skipped = 0;
                current_procs = getprocs(0);
                ...
                {
                    /**
                     * Permitir o crescimento do número de serviços
                     * nesse meio tempo.
                     */
                    adjusted_procs = current_procs + 128;
                ...
                {
                    /**
                     * Adicione um para evitar muitos procedimentos.
                     */
                ...
                /**
                 * Classifique os procedimentos por utilização da CPU.
                 */
                for (i = 0, max_sorted = 0; i < p->processes; i++)
                {
                    /**
                     * Avançar na matriz anterior para encontrar uma
                     * correspondência.
                     */
                    ...
                    switch (show_topmode)
                    {
                        ...
                        case 5:
                            ...
                            for (j = 0; j < max_sorted; j++)
                            {
                                ...
                                {
                                    /**
                                     * Pular procedimentos com zero CPU/io.
                                     */
                                ...
                                if (cursed)
                                {
                                    ...
                                    if (show_args == ARGS_ONLY)
                                    {
                                        ...
                                        /**
                                         * Um caso não muito bom, porem longo.
                                         */
                                    ...
                                    } else
                                    {
                                        ...
                                        {
                                            COLOUR wattrset(padtop, COLOR_PAIR(5));
                                            mvwprintw(padtop, j + 3 - skipped,
                                                (COLS > 119) ? 79 : 51,
                                                (COLS > 119) ? "%8d%c%8d%c" : "%4d%c%4d%c",
                                                (int)toprio,
                                                toprio_ch,
                                                (int)topwio,
                                                topwio_ch);
                                ...
                                } else
                                {
                                    if ((cmdfound && cmdcheck(p->procs[i].pi_comm)) || (!cmdfound && ((topper[j].time / elapsed) > ignore_procdisk_threshold)))
                                    {
                                            ...
                                            p->procs[i].pi_pid,                                  // 1.
                                            LOOP,                                                // 2.
                                            topper[j].time / elapsed,                            // 3.
                                            TIMEDELTA(pi_utime, i, topper[j].  other) / elapsed, // 4.
                                            TIMEDELTA(pi_stime, i, topper[j].  other) / elapsed, // 5.
                                            p->procs[i].statm_size * pagesize / 1024UL,          // 6 - em KB.
                                            p->procs[i].statm_resident * pagesize / 1024UL,      // 7 - em KB.
                                            p->procs[i].statm_trs * pagesize / 1024UL,           // 8 - em KB.
                                            p->procs[i].statm_drs * pagesize / 1024UL,           // 9 - em KB.
                                            p->procs[i].statm_share * pagesize / 1024UL,         // 10 - em KB.
                                            (int) (COUNTDELTA(pi_minflt) / elapsed),             // 11.
                                            (int) (COUNTDELTA(pi_majflt) / elapsed),             // 12.
                                            p->procs[i].pi_comm                                  // 13.
                                            #ifdef PRE_KERNEL_2_6_18
                                                );
                                            #else
                                                ,
            ...
            } else
            {
                ...
                {
                    /**
                     * Pausar pelo número solicitado de segundos.
                     */
                    secs = seconds;
                } else
                {
                    /**
                     * Reduzir o tempo pausado por número inteiro em segundos.
                     */
                ...
                {
                    /**
                     * Verificação de especificação caso as CPUs estejam
                     * em bom uso e o nmon demore muito para ser
                     * concluído.
                     */
                    secs = 1;
                }
            ...
            if (loop >= maxloops)
            {
                ...
                if (nmon_end)
                {
                    ...
                    /**
                     * Dê ao final - fazendo o procedimento por algum
                     * tempo - 5s por enquanto.
                     */
                    sleep(5);
                }
